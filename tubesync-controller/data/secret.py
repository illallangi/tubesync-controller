...
@kopf.on.probe(
    id=data_secret_idx.__name__,
)
async def data_secret_probe(
    data_secret_idx: kopf.Index,
    **_,
):
    return {
        namespace: {
            o["metadata"]["name"]: o for o in data_secret_idx[namespace]
        }
        for namespace in data_secret_idx
    }
...
async def data_secret(
    ...
    **kwargs,
    ...
    body = (
        load(
            env.get_template("secret.yaml.j2").render(
                namespace=namespace,
                selector=label.but(
                    LABEL_CONTROLLER,
                ).asdict,
