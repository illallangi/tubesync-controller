...
@kopf.on.probe(
    id=data_configmap_idx.__name__,
)
async def data_configmap_probe(
    data_configmap_idx: kopf.Index,
    **_,
):
    return {
        namespace: {
            o["metadata"]["name"]: o for o in data_configmap_idx[namespace]
        }
        for namespace in data_configmap_idx
    }
