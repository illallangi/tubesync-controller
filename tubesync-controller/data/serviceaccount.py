...
CONTROLLER="data-serviceaccount"
...
async def data_serviceaccount_idx(
    namespace,
    body,
    **_,
):
...
@kopf.on.probe(
    id=data_serviceaccount_idx.__name__,
)
async def data_serviceaccount_probe(
    data_serviceaccount_idx: kopf.Index,
    **_,
):
    return {
        namespace: {
            o["metadata"]["name"]: o for o in data_serviceaccount_idx[namespace]
        }
        for namespace in data_serviceaccount_idx
    }
