...
async def application_role_idx(
    namespace,
    body,
    **_,
):
...
@kopf.on.probe(
    id=application_role_idx.__name__,
)
async def application_role_probe(
    application_role_idx: kopf.Index,
    **_,
):
    return {
        namespace: {
            o["metadata"]["name"]: o for o in application_role_idx[namespace]
        }
        for namespace in application_role_idx
    }
...
async def application_role(
    ...
    **kwargs,
    ...
    body = (
        load(
            env.get_template("role.yaml.j2").render(
                namespace=namespace,
                selector=label.but(
                    LABEL_CONTROLLER,
                ).asdict,
