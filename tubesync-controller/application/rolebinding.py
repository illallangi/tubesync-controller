...
CONTROLLER="application-rolebinding"
...
async def application_rolebinding_idx(
    namespace,
    body,
    **_,
):
...
@kopf.on.probe(
    id=application_rolebinding_idx.__name__,
)
async def application_rolebinding_probe(
    application_rolebinding_idx: kopf.Index,
    **_,
):
    return {
        namespace: {
            o["metadata"]["name"]: o for o in application_rolebinding_idx[namespace]
        }
        for namespace in application_rolebinding_idx
    }
...
async def application_rolebinding(
    namespace,
    body,
    patch,
    **kwargs,
    ...
    body = (
        load(
            env.get_template("rolebinding.yaml.j2").render(
                namespace=namespace,
                selector=label.but(
                    LABEL_CONTROLLER,
                ).asdict,
    ...
    obj = None
    try:
        obj = api.patch_namespaced_role_binding(
            namespace=namespace,
